...
private:
    ...
    [[noreturn]] void throwAggregatedErrors() const {
        if (errors_.empty()) {
            throw CompilerError("parser", "unknown parser error", current().location);
        }

        std::ostringstream oss;
        oss << errors_.size() << " syntax error(s):";
    ...
    std::unique_ptr<BlockNode> parseBlock() {
        ...
        skipUnsupportedTypeDeclarations();
    ...
    int parseSignedIntegerLiteral() {
        ...
        const Token number = expect(TokenKind::IntegerLiteral, "expected integer literal");
