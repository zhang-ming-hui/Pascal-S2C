...
```powershell
python .\scripts\run_regression.py --compiler .\build\Release\pascal_s2c.exe --skip-golden --parser-case SemicolonBeforeElse
```

一次跑多个 parser 错误样例：

```powershell
python .\scripts\run_regression.py --compiler .\build\Release\pascal_s2c.exe --skip-golden --parser-case SemicolonBeforeElse --parser-case WrongSemicolonAfterBegin
```
...
2. 手动运行受影响的错误样例
3. 确认新的报错行为是你想要的
4. 更新 `expected_messages.json` 中对应条目
